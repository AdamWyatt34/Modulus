...
The latest minor of the latest major receives security updates — currently the 2.1.x line. Older majors (1.x) and superseded minors (2.0.x) do not receive fixes; upgrade with `modulus upgrade --version <latest>` to stay supported. The ten `ModulusKit.*` packages are released as a coordinated set, so a security fix ships as a new version of the whole set.
