...
```powershell
# Push each tag individually — a batched push of more than three tags emits no push
# events, so GitHub starts no publish runs (see the release checklist below).
...
3. Tag all ten prefixes at the release version from that green SHA and push the tags (each tag triggers a `publish` run for its package; `--skip-duplicate` makes partial re-runs safe). **Always tag the explicit SHA** (`git tag "${prefix}<x>" <green-sha>`), never bare `git tag` against whatever HEAD happens to be — the publish workflow checks out the tag's commit and MinVer stamps the tag's version onto *that* tree, so a tag on a stale checkout ships old code under a new version number. **Push the tags one `git push` at a time** — GitHub emits no push events (and therefore starts no workflow runs) when a single push contains more than three tags, so a batched `git push origin tag1 … tag10` uploads the tags but silently publishes nothing. If that happens, delete the remote tags and re-push them individually.
