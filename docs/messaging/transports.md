...
Caveats: RabbitMQ expires messages only at the queue head, so a message with a longer TTL ahead of a shorter one delays it — with the default non-decreasing backoff this is bounded by `ConsumerRetry.MaxInterval`. With `AutoProvision=false`, pre-create `{EndpointName}.retry` with `x-dead-letter-exchange = ""` and `x-dead-letter-routing-key = {EndpointName}`. Custom transports that don't handle `MessageDispatchResult.Retry` should stay on the default `InProcess` mode.
...
**Broker-native consumer retry** (`ConsumerRetryMode.Broker`) uses Service Bus scheduled messages: on a failed pass with attempts remaining, a copy is scheduled onto the same topic with the backoff as its enqueue delay and the original is completed. Because topics cannot deliver to a single subscription, the copy fans out to every endpoint with a `modulus-redeliver-endpoint` property — endpoints it isn't meant for acknowledge it without running handlers (a small delivery cost per retry on multi-endpoint topics). `PublishScheduled` maps directly to `ScheduledEnqueueTime` with no extra topology.
