...
- **BREAKING — `IOutboxStore` claim contract (multi-instance outbox competition)**: `GetPending(batchSize, maxAttempts)` is replaced by `ClaimPending(ownerId, lease, batchSize, maxAttempts)` — a portable optimistic claim (three set-based steps, no provider-specific locking hints) that atomically stamps eligible rows with `(ClaimedBy, ClaimedUntil)` before returning them. `MarkAsProcessed`/`MarkAsFailed` gain a required leading `ownerId` and only act on rows the caller still holds the claim on; `MarkAsFailed`'s load-modify-save (which raced `Attempts` under multi-reader polling) is now a single set-based update that also releases the claim. `EfOutboxAdminStore.RetryAsync` clears a row's stale claim so an operator retry is immediately reclaimable. **Every replica can now run the outbox dispatcher**: the documented single-logical-dispatcher workaround is unnecessary — a crashed instance's claimed rows self-heal once the lease (`MessagingOptions.OutboxClaimLease`, default 5 minutes, validated ≥ 30s and > `OutboxPollInterval`) elapses. Still **at-least-once** delivery, not exactly-once — pair with the inbox for consumer-side dedup.
- **BREAKING — outbox schema**: `OutboxMessages` gains nullable `ClaimedBy` (max 100) and `ClaimedUntil`; the polling index becomes `{ProcessedAt, NextAttemptOnUtc, ScheduledOnUtc, ClaimedUntil, CreatedAt}`. Consumer-owned migration (`src/Modulus.Messaging/Migrations/README.md`); rows with `NULL` claim columns are simply unclaimed.
- **BREAKING — `RequestHandlerDelegate<TResponse>` gains a `CancellationToken` parameter** (`(CancellationToken cancellationToken = default)` — the MediatR-12 shape). Source-compatible for existing `await next()` call sites, binary-breaking for the delegate type: custom behaviors recompile against 4.0. The mediator threads the token through the pipeline instead of closure-capturing it. **Convention**: a bare `next()` flows the token the current behavior itself received (never `CancellationToken.None`); `next(someToken)` substitutes that token for every inner behavior and the handler — enabling timeout/linked-token behaviors that were impossible before. All six built-in behaviors pass their token explicitly. Caveat: an explicitly-passed `default` is indistinguishable from an omitted argument.
...
- **BREAKING — dead inbox surface**: `IInboxStore.GetPending`/`MarkAsProcessed` and `InboxMessage.ProcessedOnUtc` are gone — nothing called them; the reservation model (`TryReserve`/`MarkConsumerProcessed`, stale takeover) is the real crash-recovery path and retention purges by `OccurredOnUtc`. Custom `IInboxStore` implementations drop the members. **Schema**: `InboxMessages` loses the `ProcessedOnUtc` column and the `{ProcessedOnUtc, OccurredOnUtc}` index (the standalone `OccurredOnUtc` retention index stays) — consumer-owned migration.
